...
class AnnotationReader:
    ...
    Args:
        ...
        annotation_type : Types of annotation to be read, including 'TumourContour','MitoticFigure'
    '''
    ...
    def ReadTumourContour(self):
        ...
        if self.annotation_type != 'TumourContour':       
            ...
            self.coords_file['label'] = labels
            self.coords_file.to_csv('tumourous{}_{}.csv'.format(self.coords_file['patient_id'][0],self.dim[0]),index=False) 
    ...
    def ReadMitoticFigure(self):   
        ...
        if self.annotation_type != 'MitoticFigure':       
            ...
            df1['labels'] = [1] * df1.shape[0]            #Label 1: Mitotic Figures

            df2['x_min'] = np.array(x_min2) 
            df2['x_max'] = np.array(x_max2)
            df2['y_min'] = np.array(y_min2)
            df2['y_max'] = np.array(y_max2)
            df2['labels'] = [2] * df2.shape[0]            #Label 2: Mitotic-like figures
            ...
            def get_top_left(df):
                ...
                df['top_left_x'] = np.array(coords_m)[:,0]
                df['top_left_y'] = np.array(coords_m)[:,1]
            ...
            df.to_csv('mitosis{}_{}.csv'.format(self.coords_file['patient_id'][0],self.dim[0]),index=False) #Patches with Mitotic Figures
