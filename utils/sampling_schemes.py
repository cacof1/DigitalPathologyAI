...
def sample_n_per_sample_per_label_and_equalize(coords_file, target=None, n_per_sample=np.Inf, train_size=None,
    ...
    for li in range(len(all_labels)):
        cl = all_labels[li]
        df_per_label = coords_file[coords_file[target] == cl]
    ...
    coords_file_sampled = pd.concat(df_list)
    ...
    X = np.arange(len(coords_file_sampled))
    y = coords_file_sampled[target].values
    X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=test_size, train_size=train_size, stratify=y)

    return coords_file_sampled.iloc[X_train], coords_file_sampled.iloc[X_test]

def sample_N_per_WSI(coords_file, n_per_sample=np.Inf):
