...
class Visualize:
    ...
    def PCA(self,feature_file,n_components=3, vis_on_app=False):
        ...
        if n_components==3:
            df_pca = pd.DataFrame(self.embeddings['PCA'],columns=['pc1','pc2','pc3'])
            self.plot_components(method='PCA',n_components=3)
                        
        elif n_components==2:
            df_pca = pd.DataFrame(self.embeddings['PCA'],columns=['pc1','pc2'])
    ...
    def TSNE(self, feature_file,n_components):
        ...
        if n_components==3:
            df_tsne = pd.DataFrame(self.embeddings['TSNE'],columns=['tsne1','tsne2','tsne3'])
            self.plot_components(method='TSNE',n_components=3)
            
            
        elif n_components==2:
            df_tsne = pd.DataFrame(self.embeddings['TSNE'],columns=['tsne1','tsne2'])
    ...
    def SVD(self, feature_file,n_components):
        ...
        if n_components==3:
            df_svd = pd.DataFrame(self.embeddings['SVD'],columns=['svd1','svd2','svd3'])
            self.plot_components(method='SVD',n_components=3)
            
        elif n_components==2:
            df_svd = pd.DataFrame(self.embeddings['SVD'],columns=['svd1','svd2'])
