...
| Remove_BW        |   Float used to remove, for each contour defined in the parameter `Remove_BW_Contours` below, all patches whose average colour is ` >= Remove_BW * (255, 255, 255)` (fraction of white) or `<= Remove_BW * (1, 1, 1)` (fraction of black). This can be helpful to remove unwanted background patches that are incorrectly annotated.      | `float` between 0 and 1 to use, or set to `false`.           | `0.9` |
| Remove_BW_Contours        |   A list of strings specifying the contours on which the above procedure (`Remove_BW`) is applied.     | list of contours, or `false` to process all available contours.         | `['Tumour']` |
