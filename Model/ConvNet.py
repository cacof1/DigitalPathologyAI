...
class ConvNet(pl.LightningModule):
    def __init__(self, config, label_encoder=None):
        ...
        for zoom_level in self.config['BASEMODEL']['Vis']:
            ...
            if 'densenet' in config['BASEMODEL']['Backbone']:
                backbone = backbone(pretrained=config['ADVANCEDMODEL']['Pretrained'],
            ...
            self.models[zoom_level] = backbone
    ...
    def forward(self, x_dict):
        aggregated_features = []
        for zoom_level in self.config['BASEMODEL']['Vis']:
            features = self.models[zoom_level](x_dict[str(zoom_level)])
