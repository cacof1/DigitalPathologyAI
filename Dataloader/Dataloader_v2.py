...
class DataGenerator_MitotsisDetection(torch.utils.data.Dataset):
    def __init__(self,mitosis_file, dim = (256,256), vis_level = 0, mask_file=None, transform=None, target_transform = None)):
        self.transform = transform
        self.target_transform = target_transform
        self.mitosis_file = mitosis_file
        self.masks = mask_file
        self.normalizer = TorchMacenkoNormalizer()
        self.WSIPath = os.path.join(preprocessingfolder,"wsi")
    def __len__(self):
        return self.mitosis_file.shape[0]
    ...
    def __getitem__(self, i):
        ...
        patient_id = self.mitosis_file['patient_id'][i]
        wsi_object = WholeSlideImage(WSIPath + '/{}.svs'.format(patient_id))
        top_left = (self.mitosis_file['top_left_x'][i],self.mitosis_file['top_left_y'][i])
        image = np.array(wsi_object.wsi.read_region(top_left, vis_level, dim).convert("RGB"))
...
class DataModule(LightningDataModule):
    def __init__(self, batch_size = 1, task == None, coords_file = None, wsi_file = None, mitosis_file = None, train_transform = None, val_transform = None,  **kwargs):
        super().__init__()
        self.batch_size      = batch_size        
        
        if task == 'MitosisDetection':
            ids_split            = np.round(np.array([0.7, 0.2, 1.0])*len(mitosis_file)).astype(np.int32)
            self.train_data      = DataGenerator_MitotsisDetection(mitosis_file[ids_split[0]:ids_split[1]], transform = train_transform, **kwargs)
            self.val_data        = DataGenerator_MitotsisDetection(mitosis_file[ids_split[0]:ids_split[1]],  transform = val_transform, **kwargs)
            self.test_data       = DataGenerator_MitotsisDetection(mitosis_file[ids_split[0]:ids_split[1]], transform = val_transform, **kwargs)
            
            
        else:
            
            ids_split            = np.round(np.array([0.7, 0.2, 1.0])*len(coords_file)).astype(np.int32)
