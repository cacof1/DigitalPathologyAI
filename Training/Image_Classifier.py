...
coords_file = LoadFileParameter(ids, SVS_Folder, Patch_Folder)[:1000]

transform = transforms.Compose([
    transforms.ToTensor(),  # this also normalizes to [0,1].
])  

data = DataModule(coords_file, train_transform=transform, val_transform=transform, batch_size=64, inference=False, dim=(256, 256), target='tumour_label')
